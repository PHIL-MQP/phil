...
data_t UDPClient::Transaction(data_t data) {
  ...
  if (sendto(socket_fd, (uint8_t *) &data, data_t_size, 0, (struct sockaddr *) &server_address, socket_address_size)
      < 0) {
    std::cerr << "sendto failed: [" << strerror(errno) << "]" << std::endl;
  }

  ssize_t recvlen =
      recvfrom(socket_fd, (uint8_t *) &data, data_t_size, 0, (struct sockaddr *) &response_address,
               const_cast<socklen_t *>(&socket_address_size));
...
void UDPClient::RawTransaction(uint8_t *request, size_t request_size, uint8_t *response, size_t response_size) {
  ...
  if (sendto(socket_fd, request, request_size, 0, (struct sockaddr *) &server_address, socket_address_size)
      < 0) {
    std::cerr << "sendto failed: [" << strerror(errno) << "]" << std::endl;
  }
  ...
  ssize_t recvlen =
      recvfrom(socket_fd, response, response_size, 0, (struct sockaddr *) &response_address,
               const_cast<socklen_t *>(&socket_address_size));
}
